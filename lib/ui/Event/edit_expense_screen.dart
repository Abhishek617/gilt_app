...
class EditExpense_Screen extends StatefulWidget {
  // final List<AppContact> attendees;
  // String sdata;
  EditExpense_Screen({Key? key}) : super(key: key);

  @override
  State<EditExpense_Screen> createState() => _Edit_Expense_ScreenState();
}

class _Edit_Expense_ScreenState extends State<EditExpense_Screen> {
  ...
  createControllers(elist) {
    print( elist.attendees.toString());
    for (var i = 0; i < elist.attendees.length; i++) {
      myControllers.add(TextEditingController(text:  elist.attendees![i].expanse.toString()));
    }
  }
    ...
    _userStore.updateEvent(eData,id,(CreateEventResponseModel val) {
      ...
      if (val.success == true) {
        ...
        GlobalMethods.showErrorMessage(
            context, val.message ?? 'Failed', 'Update Event');
      }
    }, (error) {
      GlobalMethods.hideLoader();
  ...
  Widget build(BuildContext context) {
    ...
    print(args);
    _expensedescription.text = args.expenseDescription;
    _amount.text = args.totalExpense;
    print(expensedata);
